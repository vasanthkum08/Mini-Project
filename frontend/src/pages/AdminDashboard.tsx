...
import { HeartPulse, Users, Bed, Clock, RefreshCw, Activity, Camera, MapPin, CheckCircle2, AlertCircle, ShieldAlert, Phone, ShieldCheck, Star } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const fetchHospital = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to load assigned hospital operational data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateHospital = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setSuccessMsg('Hospital operational details updated successfully.');
        window.scrollTo({ top: 0, behavior: 'smooth' });
        setTimeout(() => setSuccessMsg(null), 5000);
      }
    } catch (err: any) {
      console.error(err);
      setError(err.response?.data?.message || 'Failed to save operational changes.');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
      setSaving(false);
    }
      ...
      <nav className="bg-[#1E3A8A] border-b border-blue-900 shadow-md sticky top-0 z-50 px-8 py-4 text-white">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-base font-black tracking-tight block">AEIS Operations Console</span>
              <span className="text-[9px] font-black text-cyan-300 block leading-none uppercase tracking-widest">Administrator Portal Desk</span>
            </div>
          </div>
            ...
            >
              <div className="h-10 w-10 rounded-xl overflow-hidden bg-white border border-blue-100 flex items-center justify-center text-blue-900 shrink-0 shadow-inner">
                {user?.profile_photo ? (
                  ...
                  <span className="text-sm font-black uppercase text-[#1E3A8A]">{user?.name?.slice(0, 2)}</span>
                )}
              </div>
              <div className="hidden sm:flex flex-col text-left text-xs font-bold leading-tight">
                ...
                <span className="text-[9px] text-cyan-300 font-black uppercase tracking-wider">{user?.role}</span>
              </div>
            </div>
            ...
            {profileDropdownOpen && (
              <div className="absolute right-0 top-14 w-80 bg-white border border-slate-150 shadow-2xl rounded-3xl p-6 z-50 text-slate-800 animate-slide-in space-y-4">
                <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                  <h4 className="text-xs font-black uppercase tracking-widest text-[#1E3A8A]">User Profile</h4>
                  <button 
                ...
                {isEditingProfile ? (
                  <form onSubmit={handleProfileUpdate} className="space-y-3.5 text-left text-xs font-semibold">
                    ...
                    <div>
                      <label className="block text-[9px] text-[#1E3A8A] font-black uppercase mb-1">Full Name</label>
                      <input 
                        type="text" 
                        value={editName}
                    ...
                    <div>
                      <label className="block text-[9px] text-[#1E3A8A] font-black uppercase mb-1">Mobile Number</label>
                      <input 
                        type="text" 
                        value={editMobile}
                  ...
                  <div className="space-y-4 text-left">
                    ...
                    <div className="space-y-2.5 text-xs font-semibold text-slate-655">
                      <div className="border-t border-slate-100 pt-2.5">
                        <span className="text-[9px] text-[#1E3A8A] font-black block uppercase mb-0.5">Email Address</span>
                        <span className="text-slate-800">{user?.email}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-[#1E3A8A] font-black block uppercase mb-0.5">Mobile Number</span>
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-6 py-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24 space-y-4">
            ...
            <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Loading Hospital Console...</p>
          </div>
        )}
          ...
          <div className="space-y-8 animate-slide-in">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm relative overflow-hidden flex flex-col lg:flex-row justify-between gap-6 items-start">
              <div className="space-y-4 max-w-2xl relative z-10">
                ...
                <div>
                  <h1 className="text-2xl font-black text-[#1E3A8A] tracking-tight leading-none mb-2">{hospital.name}</h1>
                ...
                <div className="space-y-1">
                  <span className="text-[9px] text-[#1E3A8A] font-black uppercase tracking-widest block">Active Specializations</span>
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    {Array.isArray(hospital.specialties) ? (
                      hospital.specialties.map((dep: string) => (
                        <span key={dep} className="px-2.5 py-0.5 bg-slate-50 text-slate-700 text-[10px] font-bold rounded-md border border-slate-200 uppercase tracking-wide">
                          {dep.trim()}
                        </span>
                      ))
                    ) : (
                      <span className="text-xs text-slate-400 font-bold">General Emergency Care</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-row sm:flex-col gap-4 items-end justify-between shrink-0 w-full lg:w-auto border-t lg:border-t-0 border-slate-100 pt-4 lg:pt-0 relative z-10">
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center min-w-[140px] text-center">
                  <span className="text-[9px] text-slate-450 font-black uppercase tracking-wider">AEIS Live Rating</span>
                ...
                <div className="text-right text-[10.5px] text-slate-655 font-bold leading-tight">
                  <p className="flex items-center justify-end gap-1"><Users className="h-3.5 w-3.5 text-[#2563EB]" /> Admin: {user?.name}</p>
                  <p className="flex items-center justify-end gap-1 mt-1 text-[#2563EB]"><Phone className="h-3.5 w-3.5 text-[#2563EB]" /> Hotline: {hospital.phone || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm space-y-6">
              <h3 className="text-xs font-black text-[#1E3A8A] uppercase tracking-widest border-b border-slate-100 pb-2.5">Live Operations Real-Time Telemetry</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="bg-slate-50 border border-slate-150 rounded-2xl p-5 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black text-slate-450 uppercase tracking-wider block">ER Care Team</span>
                    <Users className="h-4.5 w-4.5 text-[#2563EB]" />
                  </div>
                  <div>
                    <span className="text-2xl font-black text-slate-900 block">{availableDoctors} Available</span>
                    <div className="flex items-center gap-1.5 mt-1.5">
                      <span className={`h-2 w-2 rounded-full ${availableDoctors > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500 animate-pulse'}`} />
                      <span className={`text-[10px] font-black uppercase ${availableDoctors > 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                        {availableDoctors > 0 ? 'On-Call Operations Active' : 'No Doctors Available'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-150 rounded-2xl p-5 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black text-slate-450 uppercase tracking-wider block">ICU Beds Vacancy</span>
                    <Bed className="h-4.5 w-4.5 text-[#2563EB]" />
                  </div>
                  <div>
                    <span className="text-2xl font-black text-slate-900 block">{availableIcuBeds} Vacant</span>
                    {/* Capacity Indicator Progress Bar */}
                    <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden mt-3">
                      <div 
                        className={`h-full transition-all duration-500 ${
                          availableIcuBeds === 0 ? 'bg-rose-600' :
                          availableIcuBeds < 3 ? 'bg-amber-500' : 'bg-emerald-500'
                        }`} 
                        style={{ width: `${Math.min(100, (availableIcuBeds / 15) * 100)}%` }} 
                      />
                    </div>
                    <span className="text-[8px] text-slate-400 font-bold block uppercase mt-1">Estimated ICU Max Limit: 15</span>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-150 rounded-2xl p-5 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black text-slate-450 uppercase tracking-wider block">Avg ER Intake Wait</span>
                    <Clock className="h-4.5 w-4.5 text-[#2563EB]" />
                  </div>
                  <div>
                    <span className={`text-2xl font-black block ${
                      erWaitMinutes > 45 ? 'text-rose-600' :
                      erWaitMinutes > 20 ? 'text-amber-600' : 'text-emerald-600'
                    }`}>
                      {erWaitMinutes} Minutes
                    </span>
                    <div className="flex items-center gap-1.5 mt-2">
                      <span className={`h-2 w-2 rounded-full ${
                        erWaitMinutes > 45 ? 'bg-rose-500' :
                        erWaitMinutes > 20 ? 'bg-amber-500' : 'bg-emerald-500'
                      }`} />
                      <span className={`text-[10px] font-black uppercase ${
                        erWaitMinutes > 45 ? 'text-rose-700' :
                        erWaitMinutes > 20 ? 'text-amber-700' : 'text-emerald-700'
                      }`}>
                        {erWaitMinutes > 45 ? 'Critical Intake Delay' :
                         erWaitMinutes > 20 ? 'Moderate Wait Queue' : 'Speed Choice Optimal'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-150 rounded-2xl p-5 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black text-slate-450 uppercase tracking-wider block">Queue size</span>
                    <Activity className="h-4.5 w-4.5 text-[#2563EB]" />
                  </div>
                  <div>
                    <span className="text-2xl font-black text-slate-900 block">{queueCount} Patients</span>
                    <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden mt-3">
                      <div 
                        className={`h-full transition-all duration-500 ${
                          queueCount > 10 ? 'bg-rose-600' :
                          queueCount > 5 ? 'bg-amber-500' : 'bg-emerald-500'
                        }`} 
                        style={{ width: `${Math.min(100, (queueCount / 20) * 100)}%` }} 
                      />
                    </div>
                    <span className="text-[8px] text-slate-400 font-bold block uppercase mt-1">Live Ambulance Queue: {queueCount} in line</span>
                  </div>
                </div>
            ...
            <form onSubmit={handleUpdateHospital} className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm space-y-8">
              ...
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-4 border-b border-slate-150">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-black text-[#1E3A8A] uppercase tracking-tight">Operational Variables Editor</h3>
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider mt-0.5">Edit live metrics that directly influence the AI hospital sorting engine</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs font-bold">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider">Available Doctors</label>
                  <p className="text-[10px] text-slate-400 font-semibold mb-2">ER Care Team available now</p>
                  <input
                    type="number"
                    value={availableDoctors}
                    onChange={(e) => setAvailableDoctors(parseInt(e.target.value) || 0)}
                    min="0"
                ...
                {/* 2. ER Waiting Time */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider">Estimated Waiting Time</label>
                  <p className="text-[10px] text-slate-400 font-semibold mb-2">Average ER wait duration (Minutes)</p>
                  <input
                    type="number"
                    value={erWaitMinutes}
                    onChange={(e) => setErWaitMinutes(parseInt(e.target.value) || 0)}
                    min="0"
                    className="w-full border border-slate-200 bg-white rounded-xl p-3 text-xs font-black focus:outline-none focus:ring-2 focus:ring-[#2563EB]/25"
                    required
                  />
                ...
                {/* 3. Queue Count */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider">Queue Count</label>
                  <p className="text-[10px] text-slate-400 font-semibold mb-2">Number of patients currently in line</p>
                  <input
                    type="number"
                    value={queueCount}
                ...
                {/* 4. ICU Beds */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider">Available ICU Beds</label>
                  <p className="text-[10px] text-slate-400 font-semibold mb-2">Vacant critical ICU beds</p>
                  <input
                    type="number"
                ...
                {/* 5. General Beds */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider">Available General Beds</label>
                  <p className="text-[10px] text-slate-400 font-semibold mb-2">Vacant general ward beds</p>
                  <input
                    type="number"
                    value={availableGeneralBeds}
                    onChange={(e) => setAvailableGeneralBeds(parseInt(e.target.value) || 0)}
                    min="0"
                    className="w-full border border-slate-200 bg-white rounded-xl p-3 text-xs font-black focus:outline-none focus:ring-2 focus:ring-[#2563EB]/25"
                    required
                  />
                ...
                {/* 6. Emergency Beds */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider">Available Emergency Beds</label>
                  <p className="text-[10px] text-slate-400 font-semibold mb-2">Vacant emergency intake beds</p>
                  <input
                    type="number"
                    value={emergencyBeds}
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-2 md:col-span-3">
                  <label className="block text-[10px] text-[#1E3A8A] font-black uppercase tracking-wider mb-1">Ambulance Availability</label>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 border border-slate-200 rounded-xl">
                    <span className="text-slate-500 font-semibold text-xs">Are dispatchable ambulances currently active for AEIS routes?</span>
